...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (ddRoll) ddRoll.textContent = user.faculty_code || user.employee_id || '—';
...
function renderProfilePage() {
  ...
  if (profDept)  profDept.textContent  = user.department || 'GITAM University';
...
function achievementCard(a, fullMode) {
  ...
  const statusBadge = isDraft
    ...
    : a.status === 'verified'
      ? `<span style="display:inline-flex;align-items:center;gap:3px;background:#ecfdf5;color:#059669;font-size:11px;font-weight:600;padding:2px 8px;border-radius:20px">&#10003; Verified</span>`
      : a.status === 'rejected'
        ? `<span style="display:inline-flex;align-items:center;gap:3px;background:#fef2f2;color:#dc2626;font-size:11px;font-weight:600;padding:2px 8px;border-radius:20px">&#10007; Rejected</span>`
        : `<span style="display:inline-flex;align-items:center;gap:3px;background:#fffbeb;color:#d97706;font-size:11px;font-weight:600;padding:2px 8px;border-radius:20px">&#9679; Pending</span>`;
...
function openStudentModal(a) {
  ...
  document.getElementById('modal-body').innerHTML = `
    <div style="margin-bottom:16px">
      ...
      ${a.remarks ? `<div style="margin-top:8px;padding:9px 12px;background:#fef2f2;border:1px solid #fca5a5;border-radius:7px;font-size:12px;color:#b91c1c"><strong>Rejection reason:</strong> ${a.remarks}</div>` : ''}
    </div>
